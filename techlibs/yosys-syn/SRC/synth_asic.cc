...
struct SynthAsicPass : public ScriptPass
{
  // Global data
  //
  ...
  // Methods
  //
  SynthAsicPass() : ScriptPass("synth_asic", "Zero Asic ASIC synthesis flow") { }
  ...
  void check_options()
  {
     if (!config_file_success) {
       ...
       // used as 'synth_asic' option and not through config file. 
  ...
  void help() override
  {
	//   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
	log("\n");
	log("    synth_asic [options]\n");
	log("\n");
	log("This command runs Zero Asic ASIC synthesis flow.\n");
	log("\n");
	log("    -top <module>\n");
	...
	log("        Specifies the config file setting main 'synth_asic' parameters.\n");
        log("\n");
	...
	log("        Show the parameters set by the config file.\n");
        ...
        log("        It can be used only after performing a first 'synth_asic' synthesis pass \n");
        log("\n");
	...
	log("The following Yosys commands are executed underneath by 'synth_asic' :\n");
  ...
  void execute(std::vector<std::string> args, RTLIL::Design *design) override
  {
	...
	clear_flags();
        ...
        log_header(design, "Executing 'synth_asic'\n\n");
	...
	for (argidx = 1; argidx < args.size(); argidx++)
	  ...
	  if (args[argidx] == "-wait") {
        ...
        log_header(design, "Executing Zero Asic 'synth_asic' flow.\n");
        log_push();
  ...
  // ---------------------------------------------------------------------------
  // script (synth_asic flow) 
  // ---------------------------------------------------------------------------
  //
  // VERSION 1.0 (07/04/2025, Thierry): 
  //
  //        - as a starter, we mimic what is done in : 
  //          '.../siliconcompiler/tools/yosys/sc_synth_asic.tcl
  //
  ...
  void script() override
  {
    ...
    log("\nSilicon Compiler using 'synth_asic' Yosys plugin command\n");

    log("'Zero Asic' ASIC Synthesis Version : %s\n", SYNTH_ASIC_VERSION);
    ...
    // line options.
    // If options conflict (ex: lut_size) between the two, 'config' file 
    // setting has the priority over 'synth_asic' explicit option setting.
    //
    setup_options();
    ...
    // Extra line added versus 'sc_synth_asic.tcl' tcl script version
    //
    run("stat");
    ...
    if (!verilog_file.empty()) {
       ...
       run(stringf("write_verilog -noexpr -nohex -nodec %s", "netlist_synth_asic.verilog"));
    }
