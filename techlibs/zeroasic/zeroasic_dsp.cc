...
#include "techlibs/zeroasic/DSP/zeroasic_dsp_pm.h"
#include "techlibs/zeroasic/DSP/zeroasic_dsp48a_pm.h"
...
void zeroasic_simd_pack(Module *module, const std::vector<Cell*> &selected_cells)
{
	...
	auto g12 = [&f12,module](std::deque<Cell*> &simd12) {
		while (simd12.size() > 1) {
			...
			log("Analysing %s.%s for ZeroAsic DSP SIMD12 packing.\n", log_id(module), log_id(lane1));
	...
	auto g24 = [&f24,module](std::deque<Cell*> &simd24) {
		while (simd24.size() > 1) {
			...
			log("Analysing %s.%s for ZeroAsic DSP SIMD24 packing.\n", log_id(module), log_id(lane1));
...
void zeroasic_dsp48a_pack(zeroasic_dsp48a_pm &pm)
{
	auto &st = pm.st_zeroasic_dsp48a_pack;

	log("Analysing %s.%s for ZeroAsic DSP48A/DSP48A1 packing.\n", log_id(pm.module), log_id(st.dsp));
...
void zeroasic_dsp_packC(zeroasic_dsp_CREG_pm &pm)
{
	...
	log_debug("Analysing %s.%s for ZeroAsic DSP packing (CREG).\n", log_id(pm.module), log_id(st.dsp));
	log_debug("ffC:        %s\n", log_id(st.ffC, "--"));
...
struct ZeroAsicDspPass : public Pass {
	...
	void help() override
	{
		...
		log("to a maximum length of 20 cells, corresponding to the smallest ZeroAsic 7 Series\n");
		log("device.\n");
		log("\n");
	...
	void execute(std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto module : design->selected_modules()) {
			...
			//   SIMD feature
			if (family == "xc7")
				zeroasic_simd_pack(module, module->selected_cells());
			...
			if (family == "xc7") {
				...
				zeroasic_dsp48a_pm pm(module, module->selected_cells());
				pm.run_zeroasic_dsp48a_pack(zeroasic_dsp48a_pack);
			}
